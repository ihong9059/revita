/*
 * LoRa TX - SX1262 Transmitter (RAK4630)
 * Sends counter value every 1 second
 */
...
void sx_write_buffer(uint8_t offset, uint8_t* data, uint8_t len) {
    sx_wait_busy();
    sx_nss_low();
    uint8_t cmd = SX_CMD_WRITE_BUFFER;
    spi_transfer(&cmd, 0, 1);
...
void sx_set_pa_config(void) {
    uint8_t data[4];
    data[0] = 0x04;  // paDutyCycle
    data[1] = 0x07;  // hpMax
    data[2] = 0x00;  // deviceSel (SX1262)
    data[3] = 0x01;  // paLut
    sx_write_command(SX_CMD_SET_PA_CONFIG, data, 4);
}

void sx_set_tx_params(void) {
    uint8_t data[2];
    data[0] = LORA_TX_POWER;  // power
    data[1] = 0x04;           // rampTime (200us)
    sx_write_command(SX_CMD_SET_TX_PARAMS, data, 2);
}
...
void sx_set_dio_irq_params(void) {
    uint8_t data[8];
    uint16_t irq_mask = 0x0001;  // TxDone
...
void sx_set_tx(uint32_t timeout) {
    uint8_t data[3];
    data[0] = (timeout >> 16) & 0xFF;
    data[1] = (timeout >> 8) & 0xFF;
    data[2] = timeout & 0xFF;
    sx_write_command(SX_CMD_SET_TX, data, 3);
}
...
// ============ Transmit ============
void sx_transmit(uint8_t* data, uint8_t len) {
    sx_set_standby();
    sx_set_packet_params(len);
    sx_write_buffer(0, data, len);
    sx_clear_irq_status();

    // Enable antenna for TX
    P1_OUTSET = (1 << SX_ANT_SW_PIN);

    sx_set_tx(0);  // No timeout

    // Wait for TxDone
    while (!(sx_get_irq_status() & 0x0001)) {
        delay_ms(1);
    }
...
int main(void) {
    ...
    while (1) {
        ...
        uart_send("TX [");
        uart_send_dec(counter);
        uart_send("]\r\n");

        sx_transmit(packet, 4);
