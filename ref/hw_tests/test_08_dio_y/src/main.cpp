 ...
 *
 * Test procedure:
 * 1. Apply 12V to DIO_Y input -> should read LOW
...
void dio_isr() {
    dio_changed = true;
    dio_state = digitalRead(DIO_Y_PIN);
}

void show_status() {
    bool state = digitalRead(DIO_Y_PIN);

    debug_separator();
    debug_println("DIO_Y (P0.09) Status:");
...
void setup() {
    ...
    debug_test_header("DIO_Y 12V Input Test");

    debug_println("Pin Configuration:");
    debug_print("  DIO_Y Pin: P0.");
    debug_println(DIO_Y_PIN);
    debug_println("  Level shifter: PC817 optocoupler");
    debug_println("");

    // Configure DIO_Y as input with pull-up
    pinMode(DIO_Y_PIN, INPUT_PULLUP);

    // Attach interrupt for edge detection
    attachInterrupt(digitalPinToInterrupt(DIO_Y_PIN), dio_isr, CHANGE);

    debug_result("DIO_Y initialized", true);
...
void loop() {
    if (debug_available()) {
        ...
        if (continuous_mode) {
            ...
            last_state = digitalRead(DIO_Y_PIN);
        } else if (c == 'h' || c == 'H') {
            show_help();
        }
    }
    ...
    if (dio_changed) {
        dio_changed = false;
        debug_print("[IRQ] DIO_Y changed to: ");
    ...
    if (continuous_mode) {
        bool current = digitalRead(DIO_Y_PIN);
